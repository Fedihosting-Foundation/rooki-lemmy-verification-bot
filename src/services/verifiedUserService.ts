...
class verifiedUserService {
  ...
  codes: { code: number; lemmyUser: Person; discordUser: GuildMember }[] = [];
  ...
  async createVerificationCode(person: Person, discordUser: GuildMember) {
    let conn = await this.getConnection(person, undefined);
